...
# model0  count~period
# model1  count~period + veg
# model2  count~period + ipa 
# model3  count~period + site
# model4  count~period + ipa + veg
# model5  count~period + site + veg
# model6  count~period + site + ipa
# model7  count~period + site + ipa + veg
# model8  count~period + ipa + veg + veg*ipa
# model9  count~period + site + ipa + site*ipa
# model10  count~period + site + ipa + veg + veg*ipa
# model11  count~period + site + ipa + veg + site*ipa
# model12  count~period + site + ipa + veg + site*ipa + veg*ipa
...
capture.output(summary(model3[[1]], digits=2, prob=c(.025, .05, .055, .1, .9, .945, .95, .975)), file = "summary_BestModelCoeffs_Nov20_mean_allCIs.txt")
capture.output(summary(model1[[2]], digits=2, prob=c(.025, .05, .055, .1, .9, .945, .95, .975)), file = "summary_BestModelCoeffs_Nov20_mean_allCIs.txt", append = TRUE)
capture.output(summary(model6[[3]], digits=2, prob=c(.025, .05, .055, .1, .9, .945, .95, .975)), file = "summary_BestModelCoeffs_Nov20_mean_allCIs.txt", append = TRUE)
capture.output(summary(model9[[4]], digits=2, prob=c(.025, .05, .055, .1, .9, .945, .95, .975)), file = "summary_BestModelCoeffs_Nov20_mean_allCIs.txt", append = TRUE)


capture.output(posterior_interval(model3[[1]], prob=0.89), file = "posterior_interval_ModelCoeffs_89.txt")
capture.output(posterior_interval(model1[[2]], prob=0.89), file = "posterior_interval_ModelCoeffs_89.txt", append = TRUE)
capture.output(posterior_interval(model6[[3]], prob=0.89), file = "posterior_interval_ModelCoeffs_89.txt", append = TRUE)
capture.output(posterior_interval(model9[[4]], prob=0.89), file = "posterior_interval_ModelCoeffs_89.txt", append = TRUE)


capture.output(emmip(model3[[1]], site ~ period, CIs=TRUE, plotit=FALSE),file = "Emmips_Oct26.txt")
capture.output(emmip(model1[[2]], veg ~ period , CIs=TRUE, plotit=FALSE),file = "Emmips_Oct26.txt", append=TRUE)
capture.output(emmip(model6[[3]], ipa ~ site, CIs=TRUE, plotit=FALSE),file = "Emmips_Oct26.txt", append=TRUE)
capture.output(emmip(model9[[4]], ipa ~ site, CIs=TRUE, plotit=FALSE),file = "Emmips_Oct26.txt", append=TRUE)
...
# LOO LIST FOR SMELT, EXCLUDING MODELS WITH EELGRASS
i = 4
#loo_list_sm <- list()
#for(i in 1: length(df_list)) {
  loo_list_sm <- list(loo_model0[[i]], loo_model2[[i]], loo_model3[[i]], loo_model6[[i]], loo_model9[[i]])
#}
...
#stacking weights for smelt
stacking_wts[[i]]<- loo_model_weights(loo_list_sm)
                                      
                                      
capture.output(stacking_wts[[4]], file = "Bayesian model weights_with_period_newsmelt_b.txt")
...
# Plot PSIS
plot(loo_modelf, label_points=TRUE)
...
site_perd = list() # site + period - best model for Chinook
veg_perd = list() # veg + period - best model for Chum
site_ipa_perd = list() #site + ipa + period - best model for Herring
smelt_model = list() #site + ipa + veg + period + site*ipa - best model for Smelt
#site = list() 
#site2 = list()
#site_plus_ipa = list()
#site_plus_ipa2 = list()
...
#{
  ...
  #mutate(site= factor(site, levels = rev(c("edg", "dok", "
#}
